...
def test_canonical_source_dataset_and_week5_provenance_are_consistent() -> None:
    ...
    assert all(set(question.expected_chunk_ids) <= chunk_ids for question in questions)
    ...
    assert manifest["official_status"] == "PROVISIONAL_AI_REVIEW_ONLY"
    ...
    assert week5["dataset_sha256"] == manifest["dataset_sha256"]
    assert week5["input_chunk_sha256"] == manifest["source_chunks_sha256"]
    ...
    assert reranker["dataset_sha256"] == manifest["dataset_sha256"]
    assert reranker["corpus_sha256"] == manifest["source_chunks_sha256"]
